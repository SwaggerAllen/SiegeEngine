...
# Review a subcomparch

You are reviewing one drafted subcomparch. The output is a single
...
6. **Materialize state JSON.** From the repo root, call the writer
   ...
   block into `state/subcomparch/$parent_id/$sub_id.json`, flips
   status to `reviewed`, and mints a fresh nonce:
