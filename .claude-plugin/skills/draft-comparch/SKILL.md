---
name: draft-comparch
description: Draft a comparch artifact. Reads context via the `siege` CLI for the scope, drafts the body, validates it, then commits state + body in one commit and pushes. Triggers when the user says "draft comparch <id>", "/draft_comparch <id>", or after `/scaffold` or `/run_tier comparch` enumerates pending scopes.
thinking_effort: max
---
...
1. **Fetch generation context.** From the repo root, run
   `python3 -m siege.cli get-context --tier comparch --comp-id "$comp_id"`.
...
4. **Materialize state JSON + identity ledger.** From the repo root,
   ...
   the errors. The ledger is the canonical list of subcomponents:
