---
name: review-comparch
description: Review a comparch draft. Reads `get_review_context` for the scope, produces a `<review>` XML block per the parser contract, writes it as review.md, updates state JSON, commits, and pushes. Triggers automatically after a `draft-comparch` or on manual `/review_comparch <id>`.
thinking_effort: max
---

# Review a comparch

You are reviewing one drafted comparch. The output is a single
...
2. **Fetch review context.** Call
   `mcp__siegeengine__get_review_context(ref=$ref, tier="comparch",
   scope={"comp_id": $comp_id, "tier": "comparch"}, draft_sha=<draft.body_sha256 from state>)`.
...
5. **Write the review** to `comparch/$comp_id/review.md`.
6. **Materialize state JSON.** From the repo root, call the writer
   ...
   block into `state/comparch/$comp_id.json`, flips status to
   ...
   python3 -m siege.cli write-review \
     --tier comparch \
     --comp-id "$comp_id" \
     --review-path "comparch/$comp_id/review.md"
   ```
...
7. **Stage both files**, commit:
   `review(comparch/$id): score=<N> — <intro first sentence>`
8. **Push.**
