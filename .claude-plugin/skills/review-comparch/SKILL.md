...
description: Review a comparch draft. Reads context via the `siege` CLI, produces a `<review>` XML block per the parser contract, writes it as review.md, updates state JSON, commits, and pushes. Triggers automatically after a `draft-comparch` or on manual `/review_comparch <id>`.
thinking_effort: max
---
...
1. **Read the draft state.** From the repo root, run
   `python3 -m siege.cli get-state --tier comparch --comp-id "$comp_id"`.
...
2. **Fetch review context.** Run `python3 -m siege.cli
   get-review-context --tier comparch --comp-id "$comp_id"
