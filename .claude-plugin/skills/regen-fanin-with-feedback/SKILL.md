...
# Regen a fan-in synthesis with prior-review feedback

This is `draft-fanin` with the prior review text threaded in. The
...
3. **Call draft-fanin** logic with `prior_review_text` set to the
...
5. **Auto-fire the review** as a follow-up (call `review-fanin` in
...
6. **Commit + push.** Single commit:
   `regen(fanin/$id): <one-line summary> [from review score=<N>]`
