---
name: review-sysarch
description: Review a sysarch section draft. Reads context via the `siege` CLI, produces a `<review>` XML block per the parser contract, writes it as review.md, updates state JSON, commits, and pushes. Triggers automatically after a `draft-sysarch` or on manual `/review_sysarch <id>`.
thinking_effort: max
---
...
1. **Read the draft state.** From the repo root, run
   `python3 -m siege.cli get-state --tier sysarch --comp-id "$comp_id"`.
...
2. **Fetch review context.** Run `python3 -m siege.cli
   get-review-context --tier sysarch --comp-id "$comp_id"
...
6. **Materialize state JSON.** From the repo root, call the writer
   ...
   block into `state/sysarch/$comp_id.json`, flips status to
   ...
   python3 -m siege.cli write-review \
     --tier sysarch \
     --comp-id "$comp_id" \
     --review-path "sysarch/$comp_id/review.md"
   ```
