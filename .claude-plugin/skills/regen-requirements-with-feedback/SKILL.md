---
name: regen-requirements-with-feedback
description: Regenerate a requirements draft using the prior review as feedback. Reads state via the `siege` CLI to pull the prior review text, runs the same flow as `draft-requirements` but threads the review in as `prior_review_text` so the LLM addresses the findings. Triggers when the user says "regen requirements <id> from review", "/regen_requirements <id>", or as part of `/regen_below`.
thinking_effort: max
---

# Regen a requirements with prior-review feedback

This is `draft-requirements` with the prior review text threaded in. The
...
1. **Read state.** Run `python3 -m siege.cli get-state --tier requirements --comp-id "$comp_id"`. The scope must
...
3. **Call draft-requirements** logic with `prior_review_text` set to the
...
5. **Auto-fire the review** as a follow-up (call `review-requirements` in
...
6. **Commit + push.** Single commit:
   `regen(requirements/$id): <one-line summary> [from review score=<N>]`
