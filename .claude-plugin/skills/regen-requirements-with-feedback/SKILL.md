...
description: Regenerate a requirements draft using the prior review as feedback. Reads `get_state` to pull the prior review text, runs the same flow as `draft-requirements` but threads the review in as `prior_review_text` so the LLM addresses the findings. Triggers when the user says "regen requirements <id> from review", "/regen_requirements <id>", or as part of `/regen_below`.
thinking_effort: max
---
