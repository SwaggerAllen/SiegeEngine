...
# Regen a sysarch section with prior-review feedback

This is `draft-sysarch` with the prior review text threaded in. The
...
3. **Call draft-sysarch** logic with `prior_review_text` set to the
...
5. **Auto-fire the review** as a follow-up (call `review-sysarch` in
...
6. **Commit + push.** Single commit:
   `regen(sysarch/$id): <one-line summary> [from review score=<N>]`
