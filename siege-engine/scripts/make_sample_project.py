...
The v3 substrate is a git repo of artifact files (bodies + state JSON +
identity ledgers). No existing project is in that format yet, so the v3
read projections — ``build_project_graph``, ``compute_plan``, the
per-tier summaries — have nothing real to run against.

This builds one: a small subscription-billing project drafted tier by
tier through the real ``siege`` CLI write path, so the identity
ledgers, state files, and body shas are all genuine. The result is a
committed git repo a dev can point the CLI (or the dashboard server)
at:
...
two domain components. The foundation and presentational components are
left without a comparch on purpose — they project as ``status:
...
def _run_cli(argv: list[str]) -> None:
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        rc = cli_main(argv)
    if rc != 0:
        raise SystemExit(f"siege {argv[0]} failed (rc={rc}); argv={argv}")
...
def build(dest: Path) -> None:
    ...
    print(f"sample v3 project written to {dest}")
    print(f"  inspect: python -m siege.cli get-project-graph --repo {dest}")
