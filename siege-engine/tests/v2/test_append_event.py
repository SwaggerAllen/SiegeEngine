    ...
    def test_monotonic_within_project(self, db, project):
        ...
        for i in range(5):
            offset = append_event(
                db,
                project.id,
                ev.NodeCreated(node_id=f"comp_NN{i:06d}", tier="comp", kind="domain", name=f"N{i}"),
            )
...
class TestTransactionalRollback:
    def test_failed_apply_does_not_leave_event_row(self, db, project):
        ...
        before_count = db.query(GraphEvent).filter(GraphEvent.project_id == project.id).count()
        ...
        after_count = db.query(GraphEvent).filter(GraphEvent.project_id == project.id).count()
