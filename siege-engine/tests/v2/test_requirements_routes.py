...
class TestResponsibilitiesList:
    def test_empty_initially(self, client, project_with_reqs):
        resp = client.get(f"/api/projects/{project_with_reqs.id}/responsibilities")
        assert resp.status_code == 200
        assert resp.json() == {"responsibilities": []}

    def test_lists_minted_responsibilities(self, client, project_with_reqs, db):
        # Fake-mint a resp_* directly (bypassing the mint handler
        # for this particular test)
        for i, (n, intent) in enumerate(
            [("Auth", "Identify."), ("Billing", "Bill."), ("Telemetry", "Log.")]
        ):
            resp_id = mint(db, Kind.RESP)
            append_event(
                db,
                project_with_reqs.id,
                ev.NodeCreated(
                    node_id=resp_id,
                    tier="resp",
                    kind="domain",
        ...
        assert all(r["id"].startswith("resp_") for r in body["responsibilities"])
