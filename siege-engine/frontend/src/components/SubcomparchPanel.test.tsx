...
describe('SubcomparchPanel', () => {
  ...
  //   - invokes approveDraft scoped to (projectId, parentCompId, subId)
  //   - sends feedback scoped to the (parent, sub) pair
  //   - rejects with empty feedback scoped to (projectId, parentCompId, subId)
  // The scoped-mutation contract still exists on the API module, but
  // the panel has no buttons that trigger it; assert via the API tests
  // instead once the MCP read endpoints land.
  it('renders pending draft body with four sections and dep list', async () => {
