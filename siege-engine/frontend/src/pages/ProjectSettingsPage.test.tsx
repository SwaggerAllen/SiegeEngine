...
describe('ProjectSettingsPage', () => {
  it('loads the current timeout converted to minutes', async () => {
    mockedGet.mockResolvedValue({ generation_timeout_seconds: 900 });
    renderPage();
  ...
  it('converts minutes back to seconds when saving', async () => {
    mockedGet.mockResolvedValue({ generation_timeout_seconds: 900 });
    mockedUpdate.mockResolvedValue({ generation_timeout_seconds: 1200 });
    renderPage();
    const input = (await screen.findByLabelText(/Generation timeout/i)) as HTMLInputElement;
    fireEvent.change(input, { target: { value: '20' } });
    fireEvent.click(screen.getByRole('button', { name: /Save/i }));
    await waitFor(() =>
      expect(mockedUpdate).toHaveBeenCalledWith('proj_1', {
