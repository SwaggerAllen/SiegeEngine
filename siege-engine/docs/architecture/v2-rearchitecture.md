...
- Each responsibility maps to exactly one **domain component** (many responsibilities per component, but one domain component per responsibility). A responsibility may *additionally* appear in one presentational component, but only if that presentational component has a `domain_parent` edge to the domain component that owns the responsibility. This means a responsibility appears in either 1 component (domain only) or 2 components (domain + its presentational counterpart). The domain assignment is the primary; the presentational assignment is a mirror that must follow the domain-parent edge.
...
- `manifest` — file-territory manifest, one per architectural scope (project, comp, subcomp). Each scope's manifest claims paths within its slot; refs and impls override folder claims via the specificity-wins rule (see *Code generation territory*).
...
- `ref` — a reference document: a standalone supplemental-content node (DSL spec, runbook, invariants doc) generated + regenerated through the four-state panel but never frozen after approval because it mints no children. Always has `parent_id = null`; consumed by other nodes via `reference` edges rather than by scope walks. See *Project references*.

Fragment IDs extend this as described above.

Project-level singletons (`expansion`, `reqs`, `sysarch`) use the `<kind>_<8 chars>` form for consistency even though the suffix is decorative for a one-per-project node. `subreqs_*`, `fanin_*`, `vocab_*`, and `manifest_*` are *not* singletons — there's one per top-level component for `subreqs`, one per domain component with subcomponents for `fanin`, many per project for `vocab`, and one per architectural scope for `manifest` (project / comp / subcomp). The suffix is load-bearing for them and the same form applies uniformly. Uniform IDs mean uniform fragment keys, uniform lookup, no special cases at call sites.
...
- **`reference`** — `<any_node> → <any_node>`. "The source node includes the target's rendered content in its regen-context prompt." A general-purpose advisory-context edge — not ref-specific. Any node can declare that it wants to see any other node's content during regen. The context assembler walks outgoing `reference` edges from the regen target and dispatches on each target's tier to pull the right chunk (`ref_*` → full body rendered to prose, `comp_*` → the `pubapi` fragment, `policy_*` → the rationale, etc.). Incoming `reference` edges also contribute context in the reverse direction, same as dependency edges surface dependent diffs. **The `reference` edge graph must be acyclic** — cycle detection runs at edge-create time, same as for `dependency` edges, and a proposed edge that would close a cycle is rejected. Emitted by `CreateReference` and `AddReference`; removed by `RemoveReference`. Staleness propagation across `reference` edges lands with the broader staleness ledger; until then, edits are silently non-propagating, same as comp→comp dep changes today.
...
Projects sometimes ship first-class supplemental content — a DSL spec for a bundle's custom grammar, a deployment runbook, a cross-component invariants document, a design-rationale memo that multiple components need to stay consistent with. None of that fits the component tree (specs have no public surface, no responsibilities, no children), the vocabulary layer (they're not term definitions), or the fragment machinery (fragments are subordinate to an owning arch doc and cascade on merge / split of their owner). **Reference documents** get their own node tier — `ref_*` — that treats supplemental content as a standalone artifact with its own identity, lifecycle, and participation in the regen-context graph.

**Modeled as its own tier `ref_*`.** Standalone nodes, never a subordinate view of some other node. The reducer enforces a hard invariant: a `ref_*` node's `parent_id` is always `null`; any `NodeCreated` or `NodeReparented` whose target tier is `ref` and whose parent is non-null is rejected at event-apply time. This is a deliberate departure from vocab, which allows `parent_id = feat_*` for feature-local terms — refs don't have a feature-local case because their consumers can cross any tier boundary. The way a node says "this ref is relevant to me" is not by living under the same parent; it's by drawing a `reference_dep` edge (see below). Fragments are also excluded as a model: a fragment moves with its owner and cascades on merge, while a ref should reparent — if `BundleConfig` splits into two comps, the DSL spec shouldn't get deleted, it should end up attached to whichever comp the user picks.

**Generated and regeneratable.** Refs run through the same four-state panel as arch docs: generating → draft → review → approved. A `v2.generate_reference` job produces a draft, the user leaves feedback or approves, and the approved content lands on the node. **Unlike bootstrap nodes** (`expansion_*`, `reqs_*`, `sysarch_*`, `subreqs_*`), refs are **not frozen after approval**. The `UpdateReference(ref_id, feedback)` instruction triggers a fresh regen at any time, from any state. This is safe because refs don't mint children — the reason bootstrap nodes freeze is that their approval commits to a set of minted children that a later edit would desync, and refs have nothing downstream to desync. Authoring a ref is more like authoring a comparch doc (which is also regenerable with feedback) than like authoring a bootstrap node.

**Consumption via `reference` edges.** Refs participate in the regen-context graph through `reference` edges — the same general-purpose advisory-context edge type any node can use (see *Edge type vocabulary*). A comp that wants to see a ref's content during regen draws a `reference` edge from itself to the ref. A ref that needs upstream context for its own regen — a comp's pubapi, a policy's rationale, another ref — draws a `reference` edge from itself to that target. The context assembler walks these edges in both directions (outgoing edges pull target content, incoming edges contribute reverse context), so a single edge between a comp and a ref gives both sides context from the other. No special bidirectional semantics, no separate `uses`/`consumes` lists — just edges that the context assembler already knows how to handle.

**Edges are user-declared.** `CreateReference(seed_description, related_nodes)` takes one optional list of node IDs. The backend emits one `reference` edge per entry — `related_node → new_ref` — in the same transaction as the ref node is minted. Post-creation, `AddReference(source_id, target_id)` and `RemoveReference(source_id, target_id)` edit the edge set. At the UI layer, creating a ref from a component / feature / policy detail page pre-fills `related_nodes` with the current node.

**Content shape is parseable XML.** Each `ref_*` node's `content` field holds a `<reference>` block with two required children and one optional child, in fixed order: `<title>` (short prose, rendered as a heading at prompt-assembly time), `<body>` (free-form markdown prose — not a tight grammar, not bullet-structured, just text that the LLM both authors and reads), and optional `<see-also>` containing `<ref to="ref_..."/>` children that cross-reference other refs by stable ID. The `<see-also>` markers are human-readable annotations inside the stored content; they are **separate** from structural `reference_dep` edges, which are the real regen-context mechanism. A ref author can use either or both — `<see-also>` is a "if you're reading this, also read that" hint without forcing a regen-context dependency, while `reference_dep` actually pulls the target's content into the source's prompt.

**Render-time transformation for prompts.** Same storage-vs-render split as vocab. `<reference>` XML is stored verbatim on `Node.content`; at context-assembly time a formatter walks the stored XML and renders it as "# Title\n\nBody text..." prose. The LLM sees readable markdown without paying prompt tokens for raw tags. Decoupling storage from prompt format means extending the grammar later (adding `<category>`, `<applies-to-bundle>`, or other metadata) is a formatter change rather than a stored-content rewrite.

**Context assembly.** Every regen at every tier sees the `referenced_content` partition — the rendered content of every node the regen target has an outgoing `reference` edge to. The partition has its own context-budget allocation separate from vocabulary, sibling pubapis, or policy candidates, because references are content the user has *explicitly declared this node consumes* — declared intent should not have to compete with derived context for budget. See *Regen prompt context*.

**Instruction vocabulary.** `CreateReference(seed_description, related_nodes)` creates a new ref and its initial edge set in one transaction. `UpdateReference(ref_id, feedback)` triggers a regen on an existing ref regardless of its approved state — the route and the instruction both explicitly bypass any freeze check. `AddReference(source_id, target_id)` and `RemoveReference(source_id, target_id)` edit the edge set. Deletion reuses the existing `NodeDeleted` primitive; because refs have no children, delete cascade is trivial (only the node's edges need removal). There is no `NodeReparented` operation for refs — the parent-is-null invariant makes reparent a nonsense operation, and the reducer rejects it.

**UI surfaces.** Refs are not part of the decomposition graph, the component tree, or any other structural visualization. They get their own dedicated "References" tab on the project dashboard, same two-pane layout as the Vocabulary tab — list on the left, detail + edit pane on the right. Component, feature, and policy detail pages grow a "Create reference" affordance that pre-fills `related_nodes` with the current node. Structural views stay uncluttered; references get their own first-class home without fighting the decomposition UI for pixel space.

**Refs as codebase artifacts (`territory_path` and `role`).** A ref optionally carries a `territory_path` field — a path in the project's repo where the ref's content is materialized as a file. A ref with `territory_path` set is no longer just input context for some other node's regen; it's a first-class codebase artifact, edited through the ref's own regen cycle, written to disk by the same machinery that writes impl-generated files. The complementary `role` field is a short prose label (one or two sentences) describing what the ref *is* — "base prompt loaded by the comparch tier", "tier configuration JSON loaded at boot" — distinct from the ref's `<title>` (display label) and `<body>` (full content). Together they let downstream consumers reference the file by metadata: an impl that loads `prompts/base.md` at runtime can read the role + path from regen context without ever inlining the full body.

**Render-time treatment for territory refs.** When a ref with a `territory_path` is fed to a downstream node as regen context, the **default render is metadata only** — `territory_path`, `role`, `<title>`. The full `<body>` is not included in the prompt. The downstream node learns "this file exists at this path with this role" and codes against the path; the file at runtime is the source of truth. This sidesteps the translation-corruption risk where the LLM paraphrases verbatim text on regeneration of a downstream impl. Edges that genuinely need full content inline can opt in via a per-edge `inline_content` flag, kept off by default so the conservative behavior is "the file is the contract." Refs without `territory_path` retain the existing full-body render — they're context, not codebase.

**Refs stay flat — no parent-refs.** A ref consumes other content via outgoing `reference` edges, not via a parent pointer. Composition that needs hierarchy (a base prompt plus per-tier overlays, a shared header plus per-page bodies) lives in the consumer that knows the composition rule, not in the refs primitive. Generic parent-refs would only encode "ref A's content includes ref B" without saying *why* or *how*, and the semantic concepts that need that distinction — base vs override, header vs body, primary vs see-also — are domain-specific to whatever runtime owns them. Catapult bundles, for example, model default-prompt composition at the bundle layer; SiegeEngine's refs stay generic.

**Out of scope for the initial ref layer:** LLM-driven `reference` edge declaration (where the comparch LLM emits ref edges in its arch doc output rather than the user declaring them manually); a project-level "always visible" bucket that bypasses explicit edges; staleness propagation across `reference` edges (handled by the broader staleness work); cross-reference linking in rendered artifact prose (a parser that scans prose for ref titles and linkifies them); LLM-discovered references (where the LLM notices a recurring concept and surfaces it as a candidate ref for review); `<see-also>`-to-edge synchronization (automatically emitting `reference` edges from `<see-also>` markers). All straightforward follow-ups; none load-bearing for the MVP.
...
- **Referenced content** — the rendered content of every node this regen target has an outgoing `reference` edge to, pulled by walking that edge set only and dispatching on each target's tier to extract the right chunk (`ref_*` → full `<reference>` body rendered to prose, `comp_*` → the `pubapi` fragment, `policy_*` → the rationale, etc.). No reachability walk. Own context-budget partition alongside vocabulary and sibling pubapis, because references are content the user has explicitly said this node consumes — declared intent shouldn't compete with derived context for budget. See *Project references*.
...
Because each responsibility has exactly one domain component (and at most one presentational mirror), we can group all diffs touching a component and review them together. **Review pass = component**, not feature, not node.
...
To support both shapes, the project carries **file manifests** (`manifest_*` nodes) that map files and folders to the components/subcomponents that own them. A manifest is the authoritative answer to "what is this file's owning component?" — an impl node's territory is computed by querying the manifests for entries owned by that impl's component/subcomponent chain.
...
**Manifests live at every architectural scope.** A single project-wide manifest doesn't model the work — frameworks dictate folder structure differently at each level, so each scope's manifest carries the constraints relevant to that scope. The project-level manifest claims root paths (`pyproject.toml`, `lib/`, `priv/`, `test/`); a comp-level manifest claims paths inside the comp's folder; a subcomp-level manifest claims paths inside its subcomp's slot. Each manifest is generated locally from its scope's architectural state, with no walk over the whole project required to do its job. Adding a new ref or comp invalidates only the manifests at the relevant scope, not every manifest in the tree.

**Specificity wins.** Refs and impls hold file-level claims; comps and subcomps hold folder-level claims. File-level claims **override** folder-level claims, so a ref with `territory_path = lib/auth/prompts/base.md` pokes a hole in the auth comp's `lib/auth/` claim without the comp manifest having to enumerate the exception. The precedence is uniform across scopes: at the project level, comp folder claims override the root catch-all; at the comp level, subcomp folder claims override the comp catch-all; refs and impls carve out specific files at any level. Same-specificity collisions — two impls or two refs claiming the same path — are validation errors caught by the global cross-cut pass.

**Global cross-cut pass.** Local manifest generation produces independent territory claims per scope; one global pass walks the union of all `territory_path`-bearing nodes (refs + impls + comp folder claims + subcomp folder claims) and validates the unified file tree. The global pass owns three project-wide invariants: collision detection across same-specificity claims, orphan detection (paths in the repo with no claimant), and disk-vs-content drift on territory-bearing refs (the on-disk file no longer matches the ref's content, suggesting an out-of-band edit). Adding a new ref with territory invalidates only the global pass, not any per-scope manifest.

Properties of manifests:

- **One per architectural scope.** Project-level manifest plus a manifest per comp and subcomp. Each is regenerated when its scope's architectural state changes (children added, removed, renamed, or reparented within the scope) — not on unrelated edits elsewhere in the tree.
- **Reviewable like any other node**, with prose feedback and the normal approval flow. The user needs to be able to tell the LLM "no, controllers belong to the presentational layer, not the domain component" and have it stick at the relevant manifest.
- **Not a hard lock.** A manifest can be edited after code generation has already run; the next plan just has to honor the new territory. If an edit narrows a territory that already has files outside its new bounds, that's a migration the user drives through explicit edits — not something the system fixes automatically. Disk-vs-content drift on territory-bearing refs surfaces as a global-pass finding, not an automatic rewrite.
...
Implementation specifics — regeneration triggers per scope, the global-pass schedule, drift remediation flow — land during Phase 14.
...
- Per-scope file manifests (`manifest_*` — one per project / comp / subcomp) mapping files/folders to owning components; reviewable; each scope regenerated on local-scope changes. Refs and impls hold file-level claims that override comp/subcomp folder claims via specificity-wins; a global cross-cut pass catches collisions, orphans, and disk-vs-content drift.
