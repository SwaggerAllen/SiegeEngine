...
- `ref` — a reference document: a standalone supplemental-content node (DSL spec, runbook, invariants doc) generated + regenerated through the four-state panel but never frozen after approval because it mints no children. Always has `parent_id = null`; consumed by other nodes (and by other refs) via `reference_dep` edges rather than by scope walks. See *Project references*.
...
- **`reference_dep`** — `<any_node> → <any_node>`. "The source node consumes the target's content during its own regen." Two natural directions use the same edge type:
  - `<non-ref regen-running node> → ref_*`: a comp, subreqs, reqs, sysarch, impl, or plan node pulls a ref's rendered content into its regen context. The comp-reads-DSL-spec case.
  - `ref_* → <any_node>`: a ref, while regenerating, pulls the target's content as reference material. Target can be another ref, a comp (pubapi fragment), a policy (rationale), a feature / resp / subresp (content), etc. The DSL-spec-reads-BundleConfig case.
  The edge walker is generic — it walks outgoing `reference_dep` edges from any source node id and dispatches on the target's tier to pull the right chunk (`ref_*` → `Node.content` rendered to prose, `comp_*` → the `pubapi` fragment, `policy_*` → the rationale, etc.). Emitted by `CreateReference` from its `uses` / `consumes` lists and by `AddReferenceDependency`; removed by `RemoveReferenceDependency`. Feeds the regen-context assembly as a dedicated `referenced_content` partition (see *Project references* and *Regen prompt context*). Staleness propagation across `reference_dep` lands with the broader staleness ledger; until then ref edits are silently non-propagating, same as comp→comp dep changes today.
...
**Consumption is edge-scoped, not reachability-scoped.** A regen-running node sees the content of `ref_*` nodes it has an outgoing `reference_dep` edge to, and only those. There is no `parent_id` walk, no "project-level refs are visible everywhere" bucket, no content-analysis autoscoping. The appeal of implicit scoping is convenience, but it breaks the mental model: if refs are consumed via edges, they're *only* consumed via edges. If a comp needs the DSL spec, it declares an edge to it. This keeps the data model simple, the staleness ledger's future work trivial, and the user's "which ref does this comp read?" question answerable with one query.

**Refs can consume too.** `reference_dep` is symmetric in the sense that `ref_* → <any_node>` is also a valid edge shape. A ref whose own regen wants to ground its output in another ref, a component's pubapi, a policy's rationale, or a feature's intent declares an outgoing `reference_dep` edge to that target. The edge walker is generic: it walks outgoing `reference_dep` edges from any node id and dispatches on the target's tier to pull the right chunk. This keeps the DSL-spec use case honest — the spec's regen can read `BundleConfig`'s pubapi while `BundleConfig`'s comparch regen reads the spec, without introducing a second edge type or a second walker. The same machinery handles both directions, and the reducer doesn't care which tier the edge source is.

**Edges are user-declared.** `CreateReference(seed_description, uses, consumes)` takes two optional lists: `uses` is the set of nodes that will consume this new ref (backend emits one `reference_dep` per entry with `source=node, target=new_ref`), and `consumes` is the set of nodes whose content this new ref will read during its own regen (backend emits one `reference_dep` per entry with `source=new_ref, target=node`). Both lists default empty and are editable after creation via `AddReferenceDependency(source_id, target_id)` / `RemoveReferenceDependency(source_id, target_id)` — the instructions take source and target explicitly so they work uniformly in either direction (`comp → ref`, `ref → comp`, `ref → ref`). At the UI layer, creating a ref from a component / feature / policy detail page pre-fills *both* `uses = [current_node]` and `consumes = [current_node]` on the theory that if the user is creating a reference from a comp's detail page, they almost certainly want the comp to both see the ref and be seen by it. Either default is overrideable in the create dialog.
...
**Context assembly.** Every regen at every tier sees the `referenced_content` partition — the rendered content of every node the regen target has an outgoing `reference_dep` edge to. The partition has its own context-budget allocation separate from vocabulary, sibling pubapis, or policy candidates, because references are content the user has *explicitly declared this node consumes* — declared intent should not have to compete with derived context for budget. See *Regen prompt context*.

**Instruction vocabulary.** `CreateReference(seed_description, uses, consumes)` creates a new ref and its initial edge set in one transaction. `UpdateReference(ref_id, feedback)` triggers a regen on an existing ref regardless of its approved state — the route and the instruction both explicitly bypass any freeze check. `AddReferenceDependency(source_id, target_id)` and `RemoveReferenceDependency(source_id, target_id)` edit the edge set in either direction. Deletion reuses the existing `NodeDeleted` primitive; because refs have no children, delete cascade is trivial (only the node's edges need removal). There is no `NodeReparented` operation for refs — the parent-is-null invariant makes reparent a nonsense operation, and the reducer rejects it.

**UI surfaces.** Refs are not part of the decomposition graph, the component tree, or any other structural visualization. They get their own dedicated "References" tab on the project dashboard, same two-pane layout as the Vocabulary tab — list on the left grouped by scope (referenced-by-this-comp, referenced-by-this-feature, standalone), detail + edit pane on the right. Component, feature, and policy detail pages grow a "Create reference" affordance that pre-fills `uses` and `consumes` with the current node. Structural views stay uncluttered; references get their own first-class home without fighting the decomposition UI for pixel space.

**Out of scope for the initial ref layer:** LLM-driven `reference_dep` declaration (where the comparch LLM emits ref edges in its arch doc output rather than the user declaring them manually); a project-level "always visible" bucket that bypasses explicit edges; staleness propagation across `reference_dep` (handled by the broader staleness work); cross-reference linking in rendered artifact prose (a parser that scans prose for ref titles and linkifies them); LLM-discovered references (where the LLM notices a recurring concept and surfaces it as a candidate ref for review); `<see-also>`-to-edge synchronization (automatically emitting `reference_dep` edges from `<see-also>` markers). All straightforward follow-ups; none load-bearing for the MVP.
...
SiegeEngine is a meaning engine, not a document generator. Its job is to take an unstructured description of a problem and its intended solution — too large for any single LLM call to hold at once — and articulate it as a coherent structure of handles that downstream passes can reason from without re-reading upstream content. The documents the system produces are **derived views** of that structure, never the artifact itself. The structured model is the source of truth; users do not edit documents directly; every write goes through the LLM via prose feedback → regenerate → approve. Those three rules are consequences of the framing, not the framing itself.

**Every tier transition in the generation chain is one of two operations: compression or scope-bounded expansion.** Sysarch compresses `N` top-level responsibilities into fewer components, each carrying a role paragraph + api-intent + assigned-resp list that fuses the meaning of the resps it subsumes. Comparch's `pubapi` fragment compresses a component's entire interior into a handle that dependents read in constant time. Subreqs is expansion, but the scope is bounded to a single component's meaning territory — it cannot reach across component boundaries because the bound is baked into what the pass sees. The alternation compress/expand/compress/expand is why prompts stay bounded as the project grows: no tier ever reads raw upstream content once the handles above it exist, and the handles above it always exist by the time a tier runs. Passing the input doc downstream to comparch or impl is category-confused — it's the equivalent of refusing to trust the handles, which is equivalent to admitting the compression didn't do its job, in which case the fix is upstream, not downstream.

**A tier's output is good if the next tier down can make correct decisions reading only that output.** Not "is the output structurally valid." Not "does it describe the system accurately." *Can the consumer make correct decisions from it alone.* Vagueness is a failure mode only the consumer can detect, because vagueness is defined relative to a specific set of decisions the consumer needs to make. "This component handles user-facing billing concerns" is structurally valid, parses cleanly, references the right resps — and is useless as a handle because the subreqs pass reading it cannot tell what data flows through, what state is owned, what operations happen on what entities, or what the component explicitly does not cover. A good handle is one that lets the next pass work without guessing.

**The hardest quality work lives at the last compression step before implementation.** Sysarch sets trajectory; comparch sets concreteness; impl multiplies whatever slack was left above it. If comparch's handles are sharp, impl falls out cleanly — the decisions left for impl are mechanical, with only so many correct ways to exist. If comparch's handles are vague, impl doesn't fall out, it *spreads out*, and the spread is proportional to the slack the comparch layer left. That spread is where meaning drift lives, and it's invisible from the comparch layer because the comparch layer doesn't know what impl will have to decide. Weight prompt-quality effort accordingly: comparch is the single highest-leverage tier because it's the last place articulation pressure can be applied before tactics take over.

Two consequences worth naming directly:

- **Bounded prompt context is not an optimization.** It is load-bearing for the theory. A tier that needs upstream raw text to reason is a tier whose upstream handles failed; the fix is sharpening the upstream handles, never widening the downstream context window. The "no god doc at every level" rule falls out of this directly.
- **Handle-quality signals flow upward from consumers, not downward from producers.** A validator at the producing tier can catch grammar errors but cannot catch vagueness, because vagueness is a property of whether another pass can make decisions from the output. The right place to catch handle failures is the consuming tier, emitting signals back toward the tier that produced the handle. The system has the primitives for this (event log + regen) but doesn't wire them for handle-quality feedback yet; it's post-MVP territory, but it's the shape of the right answer for "how do we know our handles are any good" once prompt nudges stop being the lever.

Existing rules fall out of this framing rather than being added to it: bootstrap nodes freeze after approval because their handles have committed to a set of downstream minted children; the two-tier decomposition (reqs / subreqs) exists so policies and comparch can reference resps by stably-minted ID; the destructive-operations gate protects *meaning* from being lost to operations (delete, merge, split) that cannot be replayed from the log. When in doubt, the test is always: **what is the next tier down going to see, and is it sharp enough for that tier to make correct decisions without re-reading upstream content?** If yes, the tier is doing its job. If no, the fix is in the tier producing the vague handle, not in widening the context of the tier consuming it.
