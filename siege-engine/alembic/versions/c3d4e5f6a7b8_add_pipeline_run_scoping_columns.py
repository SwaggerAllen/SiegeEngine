...
def upgrade() -> None:
    ...
    with op.batch_alter_table('pipeline_runs', schema=None) as batch_op:
        ...
        batch_op.add_column(sa.Column(
            'start_component_key', sa.String(255),
            nullable=True
        ))
        batch_op.add_column(sa.Column(
            'regen_generated_only', sa.Boolean(),
    ...
    op.execute(
        ...
        " SET stop_point = 'END_OF_PHASE'"
        " WHERE stop_point = 'AFTER_ALL'"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "UPDATE pipeline_runs"
        " SET stop_point = 'AFTER_ALL'"
        " WHERE stop_point = 'END_OF_PHASE'"
    )
