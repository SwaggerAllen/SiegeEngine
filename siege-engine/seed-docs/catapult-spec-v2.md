...
This document is the **input to a meaning engine**. Catapult is built on SiegeEngine, which treats a structured model as source of truth and derives every downstream document as a view of that model — but Catapult's *own* input doc (this file) sits at the top of that chain, before any structure exists. Everything SiegeEngine's extraction phase knows about Catapult it knows from reading this document. There is no upstream handle to lean on; this document *is* the top of the chain, and its compression decisions set the trajectory for everything SiegeEngine will generate below it.

That has consequences for how this document is written and how it should be read — by both human readers and by SiegeEngine's extraction pass. What follows is the framing that the rest of this document operates under.

**SiegeEngine is a meaning engine, not a document generator.** Its job is to take a description of a problem and its intended solution — too large for any single LLM call to hold at once — and articulate it as a coherent structure of handles that downstream passes can reason from without re-reading upstream content. The documents SiegeEngine produces (architecture docs, impl nodes, plans, eventually code) are derived views of that structure; they are not the artifact, they are ways of looking at the artifact. The structured model is the source of truth; users do not edit documents directly; every write to the model goes through the LLM via prose feedback → regenerate → approve.

**Every tier transition in the generation chain is compression or scope-bounded expansion.** Sysarch compresses N top-level responsibilities into fewer components, each carrying a role paragraph + api-intent + assigned-resps that fuses the meaning of the resps it subsumes. Comparch's pubapi fragment compresses a component's entire interior into a handle that dependents read in constant time. Subreqs is expansion, but the scope is bounded to a single component's meaning territory — it cannot reach across component boundaries because the bound is baked into what the pass sees. The alternation compress/expand/compress/expand is why prompts stay bounded as the project grows: no tier ever reads raw upstream content once the handles above it exist, and the handles above it always exist by the time a tier runs. Passing the input doc downstream to comparch or impl is category-confused — it's the equivalent of refusing to trust the handles, which is equivalent to admitting the compression didn't do its job, in which case the fix is upstream, not downstream.

**A tier's output is good if the next tier down can make correct decisions reading only that output.** Not "is the output structurally valid," not "does it describe the system accurately." *Can the consumer make correct decisions from it alone.* Vagueness is a failure mode only the consumer can detect, because vagueness is defined relative to a specific set of decisions the consumer needs to make. "This component handles user-facing billing concerns" is structurally valid, parses cleanly, references the right resps — and is useless as a handle because the pass reading it cannot tell what data flows through, what state is owned, what operations happen on what entities, or what the component explicitly does not cover. A good handle lets the next pass work without guessing.

**The character problem concentrates on the last compression step before implementation.** Sysarch sets trajectory; comparch sets concreteness; impl multiplies whatever slack was left above it. If comparch's handles are sharp, impl falls out cleanly — the decisions left for impl are mechanical, with only so many correct ways to exist. If comparch's handles are vague, impl doesn't fall out, it *spreads out*, and the spread is proportional to the slack the comparch layer left. That spread is where meaning drift lives, and it's invisible from the comparch layer because the comparch layer doesn't know what impl will have to decide.

**What this means for reading this document.** Every component, subcomponent, and responsibility name below is a handle. Every role paragraph is a handle. Every policy rationale is a handle. Each one is trying to carry enough specificity that the tier below it (or the LLM extracting the next tier down from it) can make correct structural decisions without re-reading the part of this document above it. Generic category labels are bugs, not style choices. If the name "Flow Runner" could plausibly describe a component in a different project, the name is probably too generic for this one. If a role paragraph does not tell the reader what data flows through it, what state it owns, or what it explicitly does not cover, the role paragraph is too vague to survive the compression chain below it. Read with that test in mind; if a section does not pass it, the section is broken and should be sharpened.

**What this means for SiegeEngine's extraction pass reading this document.** Treat this document as raw material for producing handles. The meaning-engine framing is load-bearing for your own output. Cluster responsibilities by shared data ownership and shared failure modes rather than by category label. Name components with the most distinguishing aspect of what they do, not with a generic bucket that could apply to any SaaS project. Write role paragraphs that let the next tier down (subreqs, comparch) decompose without re-reading the feature list. The quality bar for your output is measured at the tier below you, not at your own consistency with this input doc.
...
### A.1.13 Project references

Some Catapult projects ship first-class supplemental content alongside the code their components will generate: a DSL spec for a bundle's custom grammar, an opinionated deployment runbook, a set of cross-component invariants that multiple components have to honor, a design-rationale memo the architecture has to stay consistent with. None of that content fits the tiers we already have. Components have public surfaces, responsibilities, and children — specs don't. Vocabulary entries are term definitions with a specific grammar — specs are prose, not terms. Fragments are subordinate to an owning arch doc and cascade on merge or split — a spec shouldn't get deleted when the component it describes is refactored. A dedicated tier for **reference documents** lets all of that content exist as first-class nodes with their own lifecycles, their own place in the audit trail, and their own participation in the regen-context graph.

Reference documents are modeled as their own node tier: `ref_*`. They are **entities, not content** — they have titles, bodies, incoming and outgoing edges, edit/review lifecycles, and stable IDs. Modeling them as a node tier rather than as a fragment on another node is what gives them all of that: fragments are sections of a larger document reviewed as part of their owner, they cascade on merge, and they can't participate in edges. Refs can, and need to.

**No parent, ever.** The reducer enforces a hard invariant: `ref_*` nodes always have `parent_id = null`. `NodeCreated` and `NodeReparented` events whose target tier is `ref` and whose parent is non-null are rejected at event-apply time. This is a deliberate contrast with vocab, which allows `parent_id = feat_*` for feature-local scope. Refs don't have a feature-local case, because their consumers can cross any tier boundary — a DSL spec might be read by the bundle-config component, by its subcomponents, by a presentational component that documents the bundle format, and by a policy that enforces DSL conformance. Rather than invent a scope that could accommodate all those cases, refs use explicit edges to declare their consumers.

**Generated and regeneratable; not frozen.** Refs run through the same four-state panel as component arch docs: generating → draft → review → approved. A `v2.generate_reference` job produces a draft from a user-supplied seed description plus the referenced-content partition (see below), the user leaves feedback or approves, and approved content lands on the node. **Unlike the bootstrap tiers** — `expansion_*`, `reqs_*`, `sysarch_*`, `subreqs_*` — refs are **not frozen after approval**. The `UpdateReference(ref_id, feedback)` instruction works on any ref in any state and triggers a fresh regen. The freeze rule on bootstrap nodes exists because their approval mints children that a later edit would desync; refs don't mint children, so the reason to freeze doesn't apply. This matches how comparch and subcomparch docs already work — approved content can be regenerated with feedback at any time.

**Consumption is edge-scoped, and bidirectional.** `reference_dep` edges declare "the source node consumes the target's content during its own regen." The source can be any regen-running node and the target can be any node whose content is useful as reference material. Two directions arise naturally and use the same edge type:

- `<non-ref regen-running node> → ref_*`. A comp's comparch regen reads a ref's content as context. This is the "BundleConfig reads the DSL spec" case.
- `ref_* → <any_node>`. A ref's own regen reads another node's content as grounding material — another ref, a component's pubapi, a policy's rationale, a feature's intent, anything. This is the "the DSL spec reads BundleConfig's pubapi so its grammar stays aligned with the real implementation" case.

At creation time, `CreateReference(seed_description, uses, consumes)` takes two optional lists — `uses` is the set of nodes that will consume this ref (incoming edges) and `consumes` is the set of nodes whose content this ref will read during its own regen (outgoing edges). The backend emits one `reference_dep` edge per entry in the same transaction as the ref node is minted. Post-creation, `AddReferenceDependency(source_id, target_id)` and `RemoveReferenceDependency(source_id, target_id)` edit the edge set in either direction uniformly — the instructions are direction-agnostic. There is no reachability walk, no project-wide "always visible" bucket, no content-analysis autoscoping. The appeal of implicit scoping is convenience, but it breaks the mental model: if refs are consumed via edges, they are *only* consumed via edges. Explicit is better than implicit.

**Content shape is parseable XML.** Each ref's `Node.content` holds a `<reference>` block with two required children and one optional child: `<title>` (short prose rendered as a heading), `<body>` (free-form markdown prose — not a tight grammar, not bullet-structured, just readable text that the LLM both authors and reads), and optional `<see-also>` containing `<ref to="ref_..."/>` children that cross-reference other refs by stable ID. The grammar is parseable, validated at authoring time, and fits the same family as component and subcomponent arch docs. `<see-also>` markers inside the stored XML are human-readable annotations, separate from structural `reference_dep` edges; a ref author can use either or both.

**Render-time transformation for prompts.** Storage is XML; prompts are prose. At context-assembly time, a formatter walks the stored `<reference>` XML and renders it as "# Title\n\nBody text..." markdown. The LLM sees readable content without paying prompt tokens for raw tags. Decoupling storage from prompt format means extending the grammar later (adding `<category>`, `<applies-to-bundle>`, or other metadata) is a formatter change rather than a stored-content rewrite.

**Context assembly.** Every regen at every tier sees the `referenced_content` partition: the rendered content of every node the regen target has an outgoing `reference_dep` edge to, pulled by walking that edge set and dispatching on each target's tier to extract the right chunk (`ref_*` → full body, `comp_*` → `pubapi` fragment, `policy_*` → rationale, etc.). The partition has its own context-budget allocation separate from vocabulary, sibling pubapis, or policy candidates — references are content the user has *explicitly declared this node consumes*, and declared intent should not have to compete with derived context for budget.

**Staleness propagation.** When the staleness ledger lands, it follows `reference_dep` edges the same way it follows comp→comp `dependency` edges: editing a ref marks every node with an incoming edge from that ref as stale, and — because the edges are bidirectional — editing a comp's pubapi marks every ref with an outgoing edge to that comp as stale too. Until the ledger lands, ref edits are silently non-propagating, the same state of affairs as comp→comp dep changes today.

**Direct user creation outside a flow.** The instruction vocabulary includes `CreateReference(seed_description, uses, consumes)` so users can add a new reference at any time — during a flow run or between flows. A reviewer who notices that a component lacks grounding in its bundle's DSL spec clicks "Create reference" from the component's detail page, fills in a one-line description of what the ref should cover, and approves the LLM-generated draft; the `uses` and `consumes` lists both default to the current component. The instruction flows through the pending-change queue like every other structural edit. `UpdateReference`, `AddReferenceDependency`, `RemoveReferenceDependency`, and `NodeDeleted` (reused for ref deletion) cover the post-creation lifecycle. There is no `NodeReparented` operation for refs — the parent-is-null invariant makes reparent a nonsense operation.

**Exclusion from structural views.** References are not shown in the decomposition graph, the component tree, or any other structural visualization. Refs are not part of the project's architectural shape — they're supplemental content the architecture draws on. The UI surfaces references through a dedicated "References" tab on the project dashboard (two-pane layout, parallel to the Vocabulary tab — list on the left, detail + edit pane on the right), and through "Create reference" affordances on component / feature / policy detail pages that pre-fill `uses` and `consumes` with the current node. Structural views stay uncluttered.

**Out of scope for the MVP:** LLM-driven `reference_dep` declaration (where a comparch LLM, during its own regen, emits `reference_dep` edges to refs it discovers it should read), a project-level "always visible" bucket that bypasses explicit edges, staleness propagation across `reference_dep` (handled by the broader staleness work), cross-reference linking in rendered artifact prose (a parser that scans for ref titles and linkifies them), LLM-discovered references (where the LLM notices a recurring concept and surfaces it as a candidate ref for review), and automatic synchronization of `<see-also>` XML entries with structural `reference_dep` edges. All straightforward follow-ups; none load-bearing for the initial ref layer.
...
**Reference material that ships with a bundle.** When a bundle ships with supplemental reference material — its own DSL spec, an opinionated deployment runbook, a set of cross-component invariants — that material lives as `ref_*` nodes per §A.1.13, not as components, fragments, or vocabulary entries. Bundle import seeds the refs into the project the same way it seeds default prompts and validators, drawing the appropriate `reference_dep` edges from the bundle-owned components to the bundle-owned refs so every regen of those components sees the reference content. Once seeded, the refs are regeneratable and editable through the normal ref lifecycle — project owners can layer per-project feedback on top of the bundle's supplemental content without forking the bundle.
