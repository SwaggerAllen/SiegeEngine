...
Cycle detection for `dependency` and `reference` edges runs at every edge-create and at every mint pass. A proposed edge that would close a cycle is rejected before it's applied, and the prompt renders the rejection back to the LLM on retry.
