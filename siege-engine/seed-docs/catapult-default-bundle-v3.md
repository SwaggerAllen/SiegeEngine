...
- **`manifest`** — file-territory mapping from repository
  paths to owning leaves. **One per architectural scope**
  (project, comp, subcomp), not a project singleton: each
  scope's manifest claims paths within its slot, with
  refs and impls overriding folder claims via the
  specificity-wins rule. Regenerated by the code-generation
  pipeline rather than authored by the user; each
  manifest lives in the event log like every other node.
  See §6 for territory.
...
`manifest_*` nodes carry the authoritative mapping from
repository paths to owning leaves. Manifests live at every
architectural scope — one per project, one per comp, one
per subcomp — because frameworks dictate folder structure
differently at each level, and each scope's manifest
carries the constraints relevant to it. Each manifest is
generated locally from its scope's state, with no walk over
the whole project required to do its job; structural
operations reshaping a scope (split, merge, reparent,
promote, demote) regenerate that scope's manifest as part
of the same flow so territory stays aligned with the tree,
without invalidating manifests at unrelated scopes.

**Specificity wins.** Refs and impls hold file-level
claims (a `territory_path` on the node points at one
specific path); comps and subcomps hold folder-level
claims. File-level claims **override** folder-level
claims, so a ref with `territory_path = lib/auth/prompts/base.md`
pokes a hole in the auth comp's `lib/auth/` claim without
the comp manifest having to enumerate the exception. The
precedence is uniform: at the project level, comp folder
claims override the root catch-all; at the comp level,
subcomp folder claims override the comp catch-all; refs
and impls carve out specific files at any level.
Same-specificity collisions — two impls or two refs
claiming the same path — are validation errors caught by
a global cross-cut pass that walks the union of all
`territory_path`-bearing nodes and validates the unified
file tree. The same global pass owns orphan detection
(paths in the repo with no claimant) and disk-vs-content
drift on territory-bearing refs.
...
### 6.6 Bundle-prescribed manifest shape (future direction)

The platform's manifest tier (§6.4) carries territory claims
that map each path to its owner. A bundle could in principle
also prescribe **shape requirements** the manifests under it
must satisfy — an "Elixir umbrella" bundle could require
comps under it to produce manifests claiming `lib/`, `test/`,
`mix.exs`; a "monorepo workspace" bundle could prescribe
`packages/<name>/` slots; a bundle for a CLI tool could
require a single `bin/` entry plus `lib/` for the
implementation. The bundle declares the shape constraint;
child comp and subcomp manifests fill the slots the bundle
opens; refs and impls carve out specific files inside.

Bundle-prescribed shape is **out of scope for v1** of the
default bundle — the default bundle ships with no shape
constraints beyond what its component vocabulary already
implies. The hook is documented here so the design intent is
preserved for when bundle authors start asking for it.
...
### 8.7 Bundle-shipped prompts as territory-bearing refs

The default bundle's generation prompts — the per-tier
LLM-facing text its scaffold reads at generation time —
ship as `ref` nodes with `territory_path` set, per platform
§A.11.5. Each tier's prompt is one ref; the `territory_path`
lands the prompt at a known location in the project repo
(`prompts/<tier>.md` by convention) where the runtime loads
it during generation. The file on disk is the contract —
the scaffold reads the prompt body from the file at runtime,
not from the ref's stored content. The stored content is the
editor-side authoring surface; disk is the execution-side
surface, kept in sync by the platform's territory-ref write
machinery.

**Composition is bundle-side, not refs-side.** The default
bundle factors prompts into a base body shared across tiers
(meaning-engine framing, output-grammar conventions,
self-check checklist) plus per-tier overlays (what this
specific tier does, what its draft grammar is for). Stitching
base and overlay is the bundle's assembly job: at runtime the
bundle loads each tier's prompt file, resolves the base
references it depends on, and composes the final prompt.
Editing the base = editing one ref. Editing an overlay =
editing one ref. There is no parent-ref machinery on the
platform side; "which ref is the base" is a default-bundle
convention encoded in the bundle's assembly logic, not a
generic refs feature.

This shape extends naturally to user-authored prompt
overlays. A project that wants to layer per-project guidance
on top of a default-bundle prompt creates a new ref
(authored, not bundle-shipped) and the assembly step
composes it into the runtime prompt alongside the bundle's
own overlays.
...
Composition between the prompts themselves — base framing
shared across tiers plus per-tier overlays — is a bundle-
assembly concern, not a refs-primitive concern. The default
bundle keeps its prompts as flat refs and composes them at
runtime; see §8.7 for the convention.
