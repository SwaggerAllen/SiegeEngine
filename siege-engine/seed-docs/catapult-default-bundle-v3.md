...
- **`manifest`** — file-territory mapping from repository
  paths to owning leaves. Declared with `role: manifest`
  (platform §A.3.1a) so the platform recognizes it as
  territory-bearing and applies the per-scope-manifest +
  specificity-wins + global-cross-cut machinery uniformly.
  One per architectural scope (project, comp, subcomp), not
  a project singleton. Regenerated by flow runs rather than
  authored directly by the user; each manifest lives in the
  event log like every other node. See §6.4 for territory.
...
The default bundle declares a `manifest` tier with
`role: manifest` (platform §A.3.1a). The platform identifies
the tier by that role flag and applies the territory
machinery — per-scope manifests, specificity-wins
precedence between file-level and folder-level claims,
collision and orphan detection, disk-vs-content drift
checks — uniformly. The bundle author declares the tier and
its draft grammar; the platform owns everything else.

Each manifest's *content* is generated per-scope at flow
time from the project's actual state — its techspec
("Elixir Phoenix app under `lib/`"), its comp roles and
dependencies, the foundation-component catch-all rule,
project conventions. Two projects on the same default
bundle land different folder shapes when their content
differs. The bundle prompts know how to read project
context and produce sensible territory claims; they don't
ship a fixed Elixir-or-anything template.

Structural operations that reshape a scope (split, merge,
reparent, promote, demote) regenerate that scope's
manifest as part of the same flow so territory stays
aligned with the tree, without invalidating manifests at
unrelated scopes.

An orphaned folder — one not claimed by any leaf — surfaces
as a global-pass finding in the admin tools. The UI flags
manifests with coverage gaps so the reviewer can either
fix the foundation that should have claimed the folder or
explicitly delete the orphaned files.
...
### 6.6 The bundle stays framework-agnostic

The default bundle deliberately ships **no folder-shape
prescription**. There is no "Elixir umbrella template," no
"monorepo workspace template," no built-in expectation that
a comp must produce `lib/` plus `test/` plus `mix.exs`.
Folder shape is a per-project, per-flow-run output — driven
by what the project says about itself in its own techspec
and comp roles, not by what the bundle decided ahead of
time.

This keeps the default bundle reusable across the language
and framework spread its tier vocabulary supports. A bundle
author who wants strong shape opinions ships a forked
variant; the default bundle stays neutral and lets the
manifest generation flow do the framework-specific work
each time, against the project at hand.

The `role: manifest` flag on the bundle's manifest tier is
the platform-side hook that makes this possible — the
platform's territory machinery is generic over what the
bundle chose to call its manifest tier, so a fork that
swaps the tier name still inherits the validation
machinery for free.
